...
const useLoadSpellsNew = (pag: Definitions.SpellPagination) => {
  ...
  const { data, isLoading, isFetching, error, fetchNextPage } =
    useInfiniteQuery(['spells_list', pag], getSpellsCallBack, {
      ...
      getNextPageParam: (lastPage: GetSpellResponse) =>
        (lastPage.data as Definitions.SpellChangeNew[]).length === pag.limit &&
        (lastPage.data as Definitions.SpellChangeNew[]).length !== 0
...
type GetSpellResponse = {
  data: Definitions.SpellChangeNew[];
  skip: string;
};
...
const getSpells = async (prop: { pageParam?: string; queryKey: any[] }) => {
  ...
  const response = await apiClient.get(
    'https://data-api.makerdao.network/v1/protocol_parameters/parameter_event?parameter=VAT.ilks.line&ilk=PSM-USDC-A&limit=10&skip=2',
    {
      headers: {
        Accept: '*',
...
export default useLoadSpellsNew;
